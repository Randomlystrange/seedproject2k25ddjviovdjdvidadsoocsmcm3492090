    ...
    const animate = () => {
      ...
      if (visible && history.length > 0) {
        ...
        for (let i = TRAIL_LENGTH - 1; i >= 0; i--) {
          ...
          // Head is the largest dot; radius decreases monotonically toward the tail
          const headRadius = isPointer ? 4 : 3;
          const radius = Math.max(0.5, headRadius * (1 - i / TRAIL_LENGTH));
